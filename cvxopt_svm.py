...
def get_qp_params(X, y, C, kernel_params):
    ...
    yy = np.outer(y, y)
    P = np.multiply(yy, k)
    G = np.concatenate((-np.identity(n, dtype=X.dtype),
                         np.identity(n, dtype=X.dtype)), axis=0)
    h = np.concatenate((np.zeros(n), C*np.ones(n)), axis=0).astype(X.dtype)
    A = np.expand_dims(y, axis=0).astype(X.dtype)
    b = np.zeros(1).astype(X.dtype)
    q = -np.ones(n).astype(X.dtype)
    ...
    assert P.dtype == q.dtype == G.dtype == h.dtype \
            == A.dtype == b.dtype == np.float64, 'outputs must be numpy floats'
...
def fit_bias(X, y, alpha, kernel_params):
    ...
    valid_X = X[is_support]
    valid_y = y[is_support]
    valid_alpha = alpha[is_support]
    b = np.mean(valid_y - np.sum(valid_alpha * valid_y * kernel_matrix(valid_X, valid_X, kernel_params), axis=1))
...
def decision_function(X, X_train, y_train, b, alpha, kernel_params):
    ...
    ###################################################################################
    valid_alpha = alpha[is_support]
    valid_X = X_train[is_support]
    valid_y = y_train[is_support]
    h = np.sum(np.expand_dims(valid_alpha, axis=1) *
               np.expand_dims(valid_y, axis=1) *
               kernel_matrix(valid_X, X, kernel_params), axis=0) + b
